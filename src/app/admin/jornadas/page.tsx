...
export default async function AdminJornadasPage({
  ...
  const estado = ESTADOS.some((e) => e.value === filtros.estado)
    ? (filtros.estado as JornadaStatus | "todas")
    : "pendiente";
  const empleadoId = filtros.empleado ?? "";
  ...
  return (
    <>
      <AdminPageHeader
        ...
        description="Revisa las jornadas que registra el equipo, con su desglose de horas ordinarias y extra, y apruébalas o recházalas."
        breadcrumb={[{ label: "Panel", href: "/admin" }, { label: "Jornadas" }]}
        action={
          <span className="inline-flex items-center gap-1.5 rounded-full border border-dashed border-line bg-white px-4 py-2 text-sm font-semibold text-graphite">
            <Clock className="h-4 w-4" />
            Métricas y gráficas: próximamente
          </span>
        }
      />
      ...
      <Card className="mt-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-graphite">
          ...
          el ajuste <code className="rounded bg-mist px-1.5 py-0.5 font-mono text-xs">jornada_config</code>{" "}
