...
| 4 | Fase 2 del proyecto — Núcleo de horas extra | ✅ Código listo | Roles ampliados, CRUD de empleados con cuentas, portal del empleado con registro de jornadas, aprobaciones y visibilidad del contenido. **Falta**: aplicar `supabase/migrations/0002_empleados_jornadas.sql` (después de la 0001) y configurar `SUPABASE_SERVICE_ROLE_KEY`. Detalle abajo. |
| 4b | Fase 2 — Tablero de métricas de horas extra | ⏳ Pendiente | Gráficas y acumulados por empleado, por periodo y por orden de trabajo; exportación. La página `/admin/jornadas` ya queda preparada para crecer. |
