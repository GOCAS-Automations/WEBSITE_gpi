...
- [x] **Inicio (`/`)**: hero, bloque "Nuestros valores", resumen de
  ...
  servicios.
- [x] **Nosotros (`/nosotros`)**: presentación de la empresa, valores, video
...
- [x] **Servicios (`/servicios`)**: hub con las dos categorías y tarjetas
  hacia los 11 servicios.
- [x] **Ficha de un servicio** (prueba al menos 2: uno del principio y uno
...
- [x] **Proyectos (`/proyectos`)**: tarjetas de proyectos realizados, con
...
- [x] **Botón flotante de WhatsApp**: visible abajo a la derecha en todas las
...
- [x] **Página 404**: entra a una URL inventada, p. ej. `/esto-no-existe`.
...
- [x] Abre Inicio, una ficha de servicio y Contacto. Con las herramientas de
  ...
  uno revisa:
  - [x] No hay scroll horizontal (la página no se corre hacia los lados).
  - [x] No hay texto que se superponga con otro elemento.
  - [x] El menú se ve y se usa bien (hamburguesa en móvil, completo en
    escritorio).
  - [x] Las imágenes no se ven cortadas ni deformadas.
- [x] Repite Inicio y Contacto en **al menos 2 navegadores de escritorio**
  (por ejemplo Chrome y Firefox, o Edge y Firefox). Mira la consola (F12 →
  Console) en cada uno: no debe haber errores en rojo.
- [x] Si tienes Mac o iPhone a mano, abre el sitio en **Safari real** — es
...
- [x] Abre el sitio en tu **celular real** (ver nota de la sección "Antes de
...
- [x] Abre Inicio. **Sin tocar el mouse**, presiona `Tab`. El primer elemento
...
- [x] Sigue presionando `Tab` por el menú y los botones: en cada parada debe
  verse un **contorno de foco** claro alrededor del elemento (nunca debe
  "perderse" el foco sin que se note dónde está).
- [x] Achica la ventana (o usa el modo de dispositivo) para que aparezca el
...
- [x] Ve a Nosotros, usa `Tab` hasta llegar a una pregunta del acordeón de
  FAQ y presiona `Enter` o `Espacio`: debe expandirse. Repite sobre la misma
  pregunta: debe colapsarse.
- [x] (Opcional) En escritorio, usa `Tab` para llegar al enlace "Servicios"
...
- [x] **Administrador**: cierra sesión, entra con la cuenta admin. Debe ver
  en el menú lateral: Dashboard, Servicios, Proyectos, Clientes, FAQ,
  Valores, Contacto y ajustes, **Equipo, Horarios y Jornadas**.
- [x] **Coordinador**: mismo acceso que admin, salvo que en Equipo no debe
  ...
  cuenta admin).
- [x] **Community Manager**: en el menú lateral debe ver solo Dashboard,
...
- [x] **Empleado**: entra por `/mi-cuenta`. Debe caer directo en el portal de
...
- [x] **Credenciales incorrectas**: en `/mi-cuenta`, intenta entrar con un
...
- [x] **Editar un servicio**: entra a Servicios, abre uno cualquiera, cambia
  ...
  hacer falta esperar.
- [x] **Subir una imagen**: en ese mismo servicio (o en Proyectos), usa el
...
- [x] **Ocultar/mostrar un elemento**: en Clientes o en FAQ, cambia el
...
- [x] **Apagar una sección del inicio**: ve a Contacto y ajustes →
  ...
  activarlo.
- [x] **Cambiar un dato de contacto**: edita un teléfono o el horario de
...
- [x] Entra a Equipo, abre la ficha de una de las cuentas de prueba que
  creaste en el Bloque 4 (usa la de rol empleado).
- [x] **Restablecer contraseña**: pulsa el botón, confirma que te muestra
...
- [x] **Desactivar**: vuelve como manager, desactiva esa misma cuenta.
  Intenta iniciar sesión con ella: debe mostrar "Tu cuenta está
  desactivada", no dejarla entrar.
- [x] **Reactivar** la cuenta y confirma que puede volver a entrar.
- [x] **Eliminar** (zona de peligro): hazlo solo al final, después de
...
- [x] Entra a Horarios. Debe mostrar el mes actual con una tabla de lunes a
  domingo.
- [x] Navega con las flechas ← → a un mes que no exista todavía: debe
  **autocrearse** clonando el mes anterior (o el horario predeterminado) y
  mostrar un aviso explicando de dónde salió.
- [x] Edita un día: cambia la hora de salida de un día laboral (por ejemplo,
...
- [x] Escribe algo en "Nota del mes" (opcional) y pulsa **Guardar horario**.
  Recarga la página (F5) y confirma que el cambio quedó guardado.
- [x] Pulsa **"Restablecer al horario predeterminado"**: la tabla debe
...
- [x] **Jornada normal**: como empleado, en `/mi-cuenta`, registra una
...
- [x] **Turno que cruza medianoche**: registra otra jornada marcando la
...
- [x] **Turno en domingo** (para ver el recargo dominical/festivo): registra
...
- [x] **Qué ve el empleado**: vuelve a entrar con la cuenta empleado, ve a
  "Mis jornadas": la aprobada debe decir "Aprobada" y la rechazada
  "Rechazada" con el motivo visible.
- [x] **Edición de pendientes**: confirma que una jornada **pendiente** se
